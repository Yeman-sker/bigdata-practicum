...
2. 老师演示命令里是在当前目录下直接引用 `file-hdfs.conf`，我操作时发现如果不写绝对路径容易找不到文件，改用绝对路径 `/opt/soft/flume/conf/file-hdfs.conf` 启动后排除了路径报错。

### 【实操结果截图】

![终端查看 HDFS 进程存活与 Flume 采集完成状态](../assets/screenshots/2026-09-08/cli_flume_hdfs_run.png)
*图 1-2：终端查看 HDFS 进程存活与 Flume 采集完成状态*

![浏览器 9870 端口查看 HDFS 采集落地的 CSV 数据文件](../assets/screenshots/2026-09-08/webui_hdfs_neuedu_log.png)
*图 1-3：浏览器 9870 端口查看 HDFS 采集落地的 CSV 数据文件*
