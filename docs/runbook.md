...
版本：Day 2 v1.1。执行者是对应 Issue 的负责人，组长汇总端到端证据。#26–#30 的实现 PR 已合并；实现可运行不等于完整验收通过。最新真实历史、GBFS、API、Sqoop 和 Flume 的结果见 [2026-09-21 集成记录](integration/2026-09-21.md)，当前 Prism 真实浏览器验收仍受工具故障阻塞。
...
python3 -m citibike.serving_export \
  ...
  --password-file "$MYSQL_PASSWORD_FILE" \
  --evidence "$DATA_DIR/export.json"
```
