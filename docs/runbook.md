...
| #30 | `npm --prefix frontend ci`；`npm --prefix frontend run dev` | 开发根地址默认共享 live 样例；运营工作台、风险/调度视图、历史回放及站点详情；`?api=1` 切换 /api 代理 |
...
| --- | --- | --- |
| 样例/真实当前库存，打开实时地图 | 数量与 API 相同，风险默认当前；切换未来不改实际库存；站点详情同时显示当前与预测 | #30/#26 |
