...
本文保留当时的决定。2026-09-17 用户选择棱镜空间后，本文的旧布局、颜色和地图动效限制由 [DESIGN.md](../../DESIGN.md) 与[前端设计](../frontend-design.md)取代；历史文字不再约束现行 UI。现行使用 [产品与交互](../product.md)、[OpenAPI](../contracts/openapi.yaml)、[风险与调度规则](../contracts/operations.md) 和 [交付计划](../plans/delivery.md)；混合 risk_type、旧 move_bikes 公式及旧排期不再作为 v1.1 实现依据。
...
- 风险：展示风险站点列表和风险详情。
- 调度：展示 Top 5 调度建议，并提供完整建议列表入口。

两个 Tab 不同时堆叠在地图上；地图始终保持主视图，侧栏不扩展为独立运营工作台。

#### 站点历史供需分析（P0）

- 点击地图站点或风险列表中的站点，打开同一侧栏内的详情抽屉，不单独创建页面。
- 默认展示该站点按星期几和小时聚合的 24 小时 inbound、outbound、net flow 曲线。
- 展示选中小时的均值和 sample_days，帮助判断历史基线是否可靠。
- 回放模式可以查看所选日期的实际小时流量；日期仍由数据库可用数据决定。
- 没有历史基线的站点显示 INSUFFICIENT_DATA，不强行补值。

#### 运营总览（P1）

- 地图顶部只显示五个 KPI：运营站点数、可用车辆数、可用空桩数、缺车风险站点数、满桩风险站点数。
- 实时模式随快照刷新；回放模式只在所选日期存在数据时显示对应统计。
- 不创建第二个运营大屏，不增加复杂图表或钻取流程；时间不足时可以整体删除。
...
7. 首页侧栏可以在风险和调度两个 Tab 之间切换，不影响地图主视图。
8. 点击站点可以打开历史供需详情；无基线时明确显示 INSUFFICIENT_DATA。
9. 地图顶部可以显示约定的运营 KPI；时间不足时该 P1 功能可移除。
