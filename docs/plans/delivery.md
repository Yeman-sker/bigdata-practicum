...
| Day 3 | 第一小时独立提交可测试 backend/Java CI；随后 seed 查询 | canonical/metadata；复用 collector 验证 station/end 发送 | 20 分钟交接；fixture DWD/聚合；先试通小样本 Sqoop 再接 ETL | JDK 17 纯规则/到期算例；接 Maven；随后建议 | UI 原型：运营工作台、风险/调度视图与站点详情；fixture 可交互、前端 CI；优先合后端入口并确定集成环境 |
