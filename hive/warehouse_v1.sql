...
USE citibike_dw;
DROP TABLE IF EXISTS dwd_trip_v1;
DROP TABLE IF EXISTS dim_station_v1;
DROP TABLE IF EXISTS dws_station_hourly_flow_v1;
DROP TABLE IF EXISTS dws_station_hour_profile_v1;
DROP TABLE IF EXISTS dws_station_od_hourly_v1;
CREATE EXTERNAL TABLE dwd_trip_v1 (
...
STORED AS PARQUET LOCATION '${hiveconf:release_root}/dwd/dwd_trip_v1'
TBLPROPERTIES ('external.table.purge'='false');
CREATE EXTERNAL TABLE dim_station_v1 (
  ...
  dataset_id STRING
) STORED AS PARQUET LOCATION '${hiveconf:release_root}/dim/dim_station_v1'
TBLPROPERTIES ('external.table.purge'='false');
CREATE EXTERNAL TABLE dws_station_hourly_flow_v1 (
...
STORED AS PARQUET LOCATION '${hiveconf:release_root}/dws/dws_station_hourly_flow_v1'
TBLPROPERTIES ('external.table.purge'='false');
CREATE EXTERNAL TABLE dws_station_hour_profile_v1 (
  ...
  dataset_id STRING
) STORED AS PARQUET LOCATION '${hiveconf:release_root}/dws/dws_station_hour_profile_v1'
TBLPROPERTIES ('external.table.purge'='false');
CREATE EXTERNAL TABLE dws_station_od_hourly_v1 (
  hour TINYINT, from_station_id STRING, to_station_id STRING, ride_count BIGINT,
  dataset_id STRING
) PARTITIONED BY (service_date DATE)
STORED AS PARQUET LOCATION '${hiveconf:release_root}/dws/dws_station_od_hourly_v1'
TBLPROPERTIES ('external.table.purge'='false');
